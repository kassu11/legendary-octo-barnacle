...
}`,pg=be`query ($page: Int, $id: Int, $type: LikeableType) {
...
}`,$g=be`query media($id: Int, $page: Int) {
  Media(id: $id) {
    id
    title {
      userPreferred
...
}`,Dc=e=>`https://api.animethemes.moe/anime?filter[has]=resources&filter[site]=AniList&filter[external_id]=${e}&include=animethemes.animethemeentries.videos.audio,animethemes.song.artists`,bg=e=>`https://api.animethemes.moe/anime?filter[has]=resources&filter[site]=MyAnimeList&filter[external_id]=${e}&include=animethemes.animethemeentries.videos.audio,animethemes.song.artists`,yg=e=>`https://api.animethemes.moe/artist/${e}?include=songs.animethemes.anime,songs.animethemes.animethemeentries.videos.audio,songs.animethemes.song.artists,resources,images`,Rc=e=>`https://api.jikan.moe/v4/anime/${e}/full`,Nc=e=>`https://api.jikan.moe/v4/manga/${e}/full`,Oc=e=>`https://api.jikan.moe/v4/anime/${e}/characters`,Fc=e=>`https://api.jikan.moe/v4/manga/${e}/characters`,wg=e=>`https://api.jikan.moe/v4/characters/${e}/full`,Uc=e=>`https://api.jikan.moe/v4/anime/${e}/staff`,kg=(e,t)=>`https://api.jikan.moe/v4/${e}?${t}`,Sg=(e,t)=>`https://api.jikan.moe/v4/seasons/${e}?${t}`,Cg=e=>`https://api.jikan.moe/v4/genres/${e}`,Ag=be`query ($id: Int, $page: Int) {
...
}`,Tg=Bc("anime"),Ig=Bc("manga"),Vc=e=>be`query ($name: String) {
  User(name: $name) {
    id
    name
    statistics {
...
}`,Eg=Vc("manga"),Lg=Vc("anime"),Gc=e=>be`query ($name: String) {
  User(name: $name) {
    id
    name
    statistics {
...
}`,Pg=Gc("manga"),Mg=Gc("anime"),Dg=be`query ($name: String) {
  User(name: $name) {
    id
    name
    statistics {
...
}`,Ng=Hc("manga"),Og=Hc("anime"),Fg=e=>be`query ($type: MediaType, $id_in: [Int], $idMal_in: [Int]) {
...
}`,Bg=be`mutation ($id: Int) {
  ToggleFollow(userId: $id) {
    id
    name
    isFollowing
  }
}`,Vg=be`query ($id: Int!, $page: Int) {
  Page(page: $page) {
    pageInfo {
      total
      perPage
...
}`,jg=be`query media($id: Int, $page: Int) {
  Media(id: $id) {
    id
    idMal
...
}`,qg=be`mutation ($id: Int, $type: LikeableType) {
...
}`,ef=be`query{genres:GenreCollection tags:MediaTagCollection{name description category isAdult}}`,tf=be`query($type:ExternalLinkMediaType){ExternalLinkSourceCollection(mediaType:$type,type:STREAMING){id url site type language color icon}}`,nf=be`query ($mediaId: Int) {
  Media(id: $mediaId) {
    id
    title {
      userPreferred
...
}`,rf=be`query ($id: Int, $name: String) {
  User(id: $id, name: $name) {
    id
    name
    previousNames {
...
}`,af=be`query ($id: Int, $type: ActivityType, $page: Int) {
  Page(page: $page, perPage: 25) {
    pageInfo {
      total
      perPage
...
`)}}}fences(t){const n=this.rules.block.fences.exec(t);if(n){const r=n[0],i=fp(r,n[3]||"",this.rules);return{type:"code",raw:r,lang:n[2]?n[2].trim().replace(this.rules.inline.anyPunctuation,"$1"):n[2],text:i}}}heading(t){const n=this.rules.block.heading.exec(t);if(n){let r=n[2].trim();if(this.rules.other.endingHash.test(r)){const i=Rr(r,"#");(this.options.pedantic||!i||this.rules.other.endingSpaceChar.test(i))&&(r=i.trim())}return{type:"heading",raw:n[0],depth:n[1].length,text:r,tokens:this.lexer.inline(r)}}}hr(t){const n=this.rules.block.hr.exec(t);if(n)return{type:"hr",raw:Rr(n[0],`
