...
import { capitalize, formatTimeToDate, formatTitleToUrl } from "../utils/formating.js";
...
function MediaCardEpisodes(props) {
  return (
    <p>
      <Switch>
...
function MediaCardEpisodeRow(props) {
      ...
      <Show when={isOwnProfile()}>
        <Show when={props.entry.media[props.maxProgressKey] === null || count() < props.entry.media[props.maxProgressKey]}>
          <button onClick={(e) => {
            ...
            // const triggerProgressIncrease = leadingAndTrailingDebounce(async (token, mediaId, newProgress, progressKey) => {
          }}>+</button>
        </Show>
      </Show>
    </>
