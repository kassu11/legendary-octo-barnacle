...
export function useVirtualSearchParams() {
  ...
  return [
    (key) => virtualSearchParams(searchParams, params, key),
    (sParams, options) => setVirtualSearchParams(searchParams, setSearchParams, params, navigate, location, sParams, options)
  ];
}
...
function setVirtualSearchParams(searchParams, setSearchParams, params, navigate, location, sParams, options) {
  const virtualObj = getVirtualObject(searchParams, params);
  const searchQueryFromObj = (...rest) => searchQueryStringFromObject(searchParams, params, location, ...rest);
  ...
  if (keyIsNullish(sParams, ...Object.keys(virtualObj))) {
    return navigate(`/search/${params.type}${searchQueryFromObj(sParams)}`, {scroll: false, ...options});
  }
  ...
  if (params.header?.match(/^(summer|fall|spring|winter)-\d+$/)) {
    ...
    return navigate(`/search/${params.type}/${header}${searchQueryFromObj(restParams, false, header)}`, {scroll: false, ...options});
  }
...
function searchQueryStringFromObject(searchParams, params, location, sParams, addVirtualParams = true, header) {
  const virtualObj = getVirtualObject(searchParams, params, header);
